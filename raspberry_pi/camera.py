...
if os.path.exists('/dev/rfcomm0') == False: #checks if the path exists (if you connected multiple times the path might be different rfcomm1, rfcomm2, etc.
    path = 'sudo rfcomm bind 00:02:5B:00:A7:12' # Mine is 00:14:03:06:2B:9C (so it would be 'sudo rfcomm bind 00:14:03:06:2B:9C'). You can find your address in the Bluetooth Devices (menu>preferences>bluetooth manager) under the device name (HC-05).
    os.system (path)
    time.sleep(1)
